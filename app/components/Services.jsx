        ...
        {serviceData.map(({ icon, title, description, link }, index) => (
          <motion.div
            ...
            key={index} className={`border border-gray-400 rounded-lg px-8 py-12 hover:shadow-black cursor-pointer
                 hover:bg-lightHover hover:-translate-y-1 duration-500  ${isDarkMode ? 'dark:hover:bg-darkHover' : 'hover:bg-lightHover'}`}>
            <Image src={icon} alt='' className='w-10' />
            <h3 className={`text-lg my-4 ${isDarkMode ? "text-white" : "text-black"}`}>{title}</h3>
            <p className={`text-sm leading-5 my-4 font-semibold ${isDarkMode ? "text-white" : "text-black"
              }`}>{description}</p>
